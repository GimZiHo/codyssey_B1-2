...
- **완료**: 기존 과제 분석 확인, 운영 지침·학습자료 작성 기준·공통 도구 정보 이관. 서비스 주제를 학습 기록으로 결정하고 자료 범위를 B1-1·B1-2로 확정. 백엔드·개발 및 배포 도구 선택(아래 기술 구성 참고). 프로젝트 환경 구성(Vite React 프로젝트, 환경변수 제외 규칙, 기본 화면 실행·빌드 검증)과 관련 학습자료 반영.
- **진행 중**: 없음.
...
도구 선택 후 Vite·React 기본 환경 설치와 실행·빌드 검증을 완료했다. 계정 생성·원격 연동·배포는 아직 수행하지 않았다.
...
- [x] React 18 이상으로 구현한다. (React 19.3.0, 개발 화면·빌드·lint 검증 완료. preview 명령은 구성했으며 브라우저 검증은 미수행.)
